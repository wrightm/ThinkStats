...
class _DictWrapper(object):
    ...
    def getKeysForValue(self, value):
        
        keysForValue = []
        for k, v in self.d.iteritems():
            if v == value:
                keysForValue.append(k)
        return keysForValue
...
def Mode(hist):
    ...
    if not isinstance(hist, Hist):
        raise TypeError('hist is not of type Hist')
    
    maxFrequency = hist.MaxLike()
    return hist.getKeysForValue(maxFrequency)

def AllMode(hist):
    """
        Returns descending list pair of value, freq
        
        Args:
            hist: Hist object
            
        Returns: list of value, freq pairs
    """
    ...
    allValueFreq = []
    for k, v in hist.GetDict().iteritems():
        allValueFreq.append((k,v))
    
    allValueFreq = sorted(allValueFreq, key=lambda x: x[1], reverse=True)
    
    return allValueFreq

def BiasedPmf(biasPmf, pmf, name, invert=False):
    ...
    if not issubclass(biasPmf.__class__, BiasPmf) and not isinstance(biasPmf, BiasPmf):
        raise TypeError('biasPmf is not type or sub type of BiasPmf')
    
    new_pmf = pmf.Copy(name)
    for value, prob in new_pmf.Items():
        if invert:
            if value:
                new_pmf.Mult(value, 1.0/value)
        else:
            new_pmf.Mult(value,biasPmf(value))
    new_pmf.Normalize()
    return new_pmf
