...
def mean_squared_error(sample_of_means, mean):
    return sum([math.pow((sample_mean - mean),2) for sample_mean in sample_of_means ]) / float(len(sample_of_means))
    
def mean_error(sample_of_means, mean):
    return sum([(sample_mean - mean) for sample_mean in sample_of_means ]) / float(len(sample_of_means))
...
def main():
    ...
    print "lambda estimator sample in the book: %s" % posterior.MostLikeli()
    t.append(posterior)
    ...
    for n in [10, 20, 40]:
        ...
        posterior = EstimateParameter(prior, sample, name)
        print "lambda estimator sample with sample size %s and param %s : %s" % (n, param, posterior.MostLikeli())
        t.append(posterior)
